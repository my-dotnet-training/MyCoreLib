...
namespace MyCoreLib.Common.Model
{
    ...
    public class BinaryTree
    {
        ...
        public void PreStackOrder()
        {
            ...
            Stack<Node> stack = new Stack<BinaryTreeNode>();
        ...
        {
            Stack<BinaryTreeNode> lstack = new Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode>();//用于存放父节点
            Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode> rstack = new Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode>();//用于存放右孩子
        ...
        {
            BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode node = _head,pre=_head;
            //pre指针指向“之前出栈节点”，如果为null有问题，这里指向头节点,因为后续遍历中头节点肯定是最后被访问的。
            Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode> stack = new Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode>();
            while (node != null || stack.Count > 0)
            {
                while (node != null)
                {
        ...
        {
            BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode node = _head;
            Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode> stack = new Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode>();
            Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode> st = new Stack<BinaryTreeNodeBinaryTreeNodeBinaryTreeNodeBinaryTreeNode>();//辅助栈将的到的逆序变为正的
            while (node != null || stack.Count > 0)
            {
                while (node != null)
                {
        ...
        {
            BinaryTreeNodeBinaryTreeNodeBinaryTreeNode node = _head;
            Queue<BinaryTreeNodeBinaryTreeNodeBinaryTreeNode> queue = new Queue<BinaryTreeNodeBinaryTreeNodeBinaryTreeNode>();
            queue.Enqueue(node);
            while (queue.Count > 0)
            {
                node = queue.Dequeue();
        ...
        public void CountLeaf(BinaryTreeNodeBinaryTreeNodeBinaryTreeNode node, ref int count)
        {
            if (node != null)
            {
        ...
        //计算节点数
        public int Count(BinaryTreeNodeBinaryTreeNodeBinaryTreeNode root)
        {
            if (root == null) return 0;
            return Count(root.Left) + Count(root.Right) + 1;
        }
        //2、计算树的高度（后序遍历）
        public int Height(BinaryTreeNodeBinaryTreeNodeBinaryTreeNode root)
        {
            int a, b;
            if (root == null) return 0;
            a = Height(root.Left);
        ...
        //3、复制二叉树（后序遍历）
        public Node CopyTree(BinaryTreeNodeBinaryTreeNodeBinaryTreeNode root)
        {
            BinaryTreeNodeBinaryTreeNodeBinaryTreeNode newroot;
            if (root == null)
            {
                newroot = null;
            }
        ...
        public static BinaryTree CreateByPre(string s)
        {
            ...
            int _count = 0;
            BinaryTreeNodeBinaryTreeNode node = tree.Head;
            Stack<BinaryTreeNodeBinaryTreeNode> stack = new Stack<BinaryTreeNodeBinaryTreeNode>();
            while (node != null || stack.Count > 0)
            {
                while (node != null)
                {
        ...
        public static BinaryTree CreateByAfter(string s)
        {
            BinaryTree tree = new BinaryTree(s);//先以层序序列初始化个树，再调整
            int _count = 0;
            BinaryTreeNode node = tree.Head;
            BinaryTreeNode pre = tree.Head; ;
            //pre指针指向“之前出栈节点”，如果为null有问题，这里指向头节点,因为后续遍历中头节点肯定是最后被访问的。
            Stack<BinaryTreeNode> stack = new Stack<BinaryTreeNode>();
            while (node != null || stack.Count > 0)
            {
                ...
                if (stack.Count > 0)
                {
                    BinaryTreeNode temp = stack.Peek().Right;//获取栈顶元素的右孩子，C#的栈因为有了这个方法使得操作简单
                    if (temp == null || temp == pre)//满足规则1
                    {
                        node = stack.Pop();//出栈进行访问
                        node.Data = s[_count++];
