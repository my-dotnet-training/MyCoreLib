...
namespace MyCoreLib.Common.Model
{
    public abstract class TaskProcessorQueueBase<T> : IDisposable
    {
        ...
        //Math.Min(32, Math.Max(4, (int)(Environment.ProcessorCount * 1.5d)))

        /// <summary>
        /// Create a new task queue
        /// </summary>
        protected TaskProcessorQueueBase(string name, int concurrency, int initialDelayMS = 2000)
        {
            ...
            {
                // Create some background tasks for processing
                // We will use at most 32 background tasks, or 4 at the minimal.
                //Utility.OutputDebugString("[{0}] Creating {1} background tasks for task processing in app domain {2}.", name, concurrency, AppDomain.CurrentDomain.FriendlyName);
