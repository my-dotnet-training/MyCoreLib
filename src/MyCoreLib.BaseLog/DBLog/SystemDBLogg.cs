...
{
    public class SystemDBLogg : BaseSingleton<SystemDBLogg>, ILog
    {
        ...
        /// Get a list of logs.
        /// </summary>
        public List<SystemDBLogg> GetLogs(int startId, int maxRows, out int minLogId, out int maxLogId, DBInstance instance = DBInstance.WeipanDB_Slave)
        {
            if (startId <= 0)
                throw new ArgumentOutOfRangeException("startId");
            if (maxRows <= 0)
